 ...
 **/
 ...
 int main ()
 {
   ...
   **/
   PID pid_steer = PID();
   pid_steer.Init(0.18, 0.015, 0.03, 1.0, -1.0); // Kp, Ki, Kd, max, min
   ...
   **/
   ...
   pid_throttle.Init(0.22, 0.008, 0.025, 1.0, -1.0); // Kp, Ki, Kd, max, min
   ...
   h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
   {
     ...
     if (s != "") {
       ...
       **/
       // Calculate error based on lateral distance to the nearest trajectory point
       ...
       // Calculate cross-track error with sign based on vehicle's orientation
       double dx = x_points[closest_idx] - x_position;
       double dy = y_points[closest_idx] - y_position;
       error_steer = atan2(dy, dx) - yaw;
       if(error_steer > M_PI) error_steer -= 2*M_PI;
       if(error_steer < -M_PI) error_steer += 2*M_PI;
       ...
       **/
       // Use average desired speed of next few points
       double desired_speed = 0;
       int look_ahead = min(3, (int)v_points.size());
       for(int j = 0; j < look_ahead; j++) {
         desired_speed += v_points[j];
       }
       desired_speed /= look_ahead;
