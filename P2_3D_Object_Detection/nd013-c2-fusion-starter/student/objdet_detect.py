...
def load_configs_model(model_name='darknet', configs=None):
    ...
    if model_name == "darknet":
        ...
        ####### ID_S3_EX1-3 START #######     
        #######
        print("Executing Task ID_S3_EX1-3")

    # Define paths and configurations for the FPN ResNet model
        ...
        configs.output_video_fn = 'output_fpn_resnet_18'
    ...
    # Define model heads using a dictionary for flexibility
        configs.heads = {
...
def detect_objects(input_bev_maps, model, configs):
    ...
    for object in detections:
        det_id, det_x, det_y, det_z, det_h, det_w, det_l, det_yaw = object
        
        ## step 3 : perform the conversion using the limits for x, y and z set in the configs structure
        x_coord = det_y / configs.bev_height * (configs.lim_x[1] - configs.lim_x[0])
        y_coord = det_x / configs.bev_width * (configs.lim_y[1] - configs.lim_y[0])
        y_coord -= (configs.lim_y[1] -configs.lim_y[0]) / 2
        width = det_w / configs.bev_width * (configs.lim_y[1] - configs.lim_y[0])
        length = det_l / configs.bev_height * (configs.lim_x[1] - configs.lim_x[0])
        height = det_h
        z_coord = det_z
        orientation = det_yaw
        
        ## step 4 : append the current object to the 'objects' array
        if ((x_coord >= configs.lim_x[0] and x_coord <= configs.lim_x[1]) and
        (y_coord >= configs.lim_y[0] and y_coord <= configs.lim_y[1]) and 
        (z_coord >= configs.lim_z[0] and z_coord <= configs.lim_z[1])):
            objects.append([1, x_coord, y_coord, z_coord, height, width, length, orientation])
