...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  for (auto wp : spiral) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      auto circle_center_x = CIRCLE_OFFSETS[c] * cos(cur_yaw) + cur_x;  // <- Update 
      auto circle_center_y = CIRCLE_OFFSETS[c] * sin(cur_yaw) + cur_y;  // <- Update 
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // obstacle/actor
          double dist = sqrt(pow((actor_center_x - circle_center_x), 2) + pow((actor_center_y - circle_center_y),2));  // <- Update
